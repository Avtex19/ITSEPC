 ...
 */
 export function chordLength(radius: number, angleInDegrees: number): number {
  ...
  return Number((2 * radius * Math.sin(angleInRadians / 2)).toFixed(10)); // Adjust precision
}
 ...
 */export function distance(p1: Point, p2: Point): number {
  return Number(
    Math.sqrt(Math.pow(p2.x - p1.x, 2) + Math.pow(p2.y - p1.y, 2)).toFixed(10)
  );
}
 ...
 */
 export function findPath(turtle: Turtle, points: Point[]): string[] {
  ...
  for (const point of points) {
    ...
    path.push(`turn ${turnAngle}`);
    path.push(`forward ${distance(currentPos, point)}`);
